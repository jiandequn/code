...
function export_sqoop(){
    ...
    --columns t_date,user_type,${retention_hive_field} \
...
poll_yarn_queue
#清除表记录
hive -e "truncate table ${hive_db}.noapp_retention_count_day;"
while [ $e_stamp -lt $edate ]; do
	...
	#计算留存
	hive --database=${hive_db} --hiveconf mapreduce.job.queuename=$yarn_queue -d startDate=$start_date -d endDate=$end_date -d dayNum=$retention_day_num -f day/noapp_retention_user/${retention_hive_file}.hive
