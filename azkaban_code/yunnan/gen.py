...
class Gen:
    ...
    def _run_exec(self,cur_node,currNodes):
        # 执行当前name
        if cur_node == None:
            return;
        ...
        for node in currNodes:
            if node.has_key("dependsOn"):
                ...
                if flag and dependOnAllRun:  # 树状结构下都依赖都执行了,才会执行子接口
                    self._run_exec(node,currNodes)
    def _foreachNode(self,runname,curr_nodes):
            for node in curr_nodes:
                if node["type"] == "flow":
                    child_nodes=node["nodes"];
                    if node["name"] == runname:#表示这个嵌套flow全部执行
                        for t_node in child_nodes:
                            if not t_node.has_key("dependsOn"):
                                self._run_exec(t_node["name"], child_nodes);
                    else:
                        self._foreachNode(runname,child_nodes);
                else:
                    if node["name"] == runname:  # 需要执行的
                         self._run_exec(node,curr_nodes);
                         break;
    ...
    def start(self):
        ...
        for run_name in self.jobs:
            self._foreachNode(runname=run_name,curr_nodes=self.nodes)
