...
CREATE TABLE online_order_count_month (
  `y` char(4)  DEFAULT NULL COMMENT '年份',
  `m` char(2)  DEFAULT NULL COMMENT '月',
  `user_type` char(3)  DEFAULT NULL COMMENT '用户类型',
  `third_product_code` varchar(255)  DEFAULT NULL COMMENT '产品Code',
  `user_count` INT DEFAULT NULL COMMENT '订购用户数',
  `order_count` INT DEFAULT NULL COMMENT '订购次数'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='在线订购月统计';

DROP TABLE IF EXISTS online_order_count_week;
CREATE TABLE online_order_count_week (
  `y` char(4)  DEFAULT NULL COMMENT '年份',
  `w` char(2)  DEFAULT NULL COMMENT '月',
  `user_type` char(3)  DEFAULT NULL COMMENT '用户类型',
  `third_product_code` varchar(255)  DEFAULT NULL COMMENT '产品Code',
  `user_count` INT DEFAULT NULL COMMENT '订购用户数',
  `order_count` INT DEFAULT NULL COMMENT '订购次数'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='在线订购周统计';

DROP TABLE IF EXISTS online_order_count_day;
CREATE TABLE online_order_count_day (
  `t_date` char(10)  DEFAULT NULL COMMENT '日期',
  `user_type` char(3)  DEFAULT NULL COMMENT '用户类型',
  `third_product_code` varchar(255)  DEFAULT NULL COMMENT '产品Code',
  `user_count` INT DEFAULT NULL COMMENT '订购用户数',
  `order_count` INT DEFAULT NULL COMMENT '订购次数'
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='在线订购日统计';