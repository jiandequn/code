...
# 获取远程获取hadoop日志数据
def getHadoopLineData(path='',sn=''):
    cmdstr="hadoop fs -cat %s"% path;
    if sn !='':
        cmdstr="%s |grep %s" %(cmdstr,sn)
    # transport = paramiko.Transport(("192.168.15.222", 22))
    # transport.connect(username="root", password="asd123");
    ssh = paramiko.SSHClient()
    ssh.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    ssh.connect(hostname="192.168.15.222",port=22,username="root",password="asd123")
    # tdin, stdout, stderr = ssh.exec_command("bash --login -c '%s'" % cmdstr)
    print cmdstr
    tdin, stdout, stderr = ssh.exec_command("bash --login -c '%s'" % cmdstr)
    ...
    if len(resultLines)>0:
        for line in resultLines:
            print line
        return resultLines
        # computeData(aggregate(resultLines))

getHadoopLineData("/data2/logs/2020/09/04/*/*",'12113500E05FF008674EC26514')
