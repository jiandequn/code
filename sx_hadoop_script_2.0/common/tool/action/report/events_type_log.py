...
class EventsTypeLog(DayBuilder):
    def start_day(self):
        ...
        if self._action._startDate.strftime("%Y-%m-%d") >= '2021-03-17': #如果执行时间在版本升级后，执行心跳计算化
            self._action._hiveUtils.exceFile(filePath="log_analysis/eventsType5.hive",
                                             hived={"startDate": self._action._startDate.strftime("%Y-%m-%d"),
                                                    "endDate": self._action._endDate.strftime("%Y-%m-%d")});
        else:
            self._action._hiveUtils.exceFile(filePath="log_analysis/eventsType1.hive",
                                             hived={"startDate": self._action._startDate.strftime("%Y-%m-%d"),
                                                    "endDate": self._action._endDate.strftime("%Y-%m-%d")});
