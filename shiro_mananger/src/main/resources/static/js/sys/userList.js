    ...
    layui.use('table', function(){
        ...
        tableIns=table.render({
            ...
            ,cols: [[
                ...
                ,{field:'del', title:'状态',width:95,align:'center',templet:function(r){
...
//开通用户
function addUser(){
    $.get("/sys/role/getAll",function(data){
        if(data!=null){
            //显示角色数据
            //$("#roleDiv").empty();
    ...
    layer.open({
        ...
        ,btn1: function(index, layero){
            ...
            layer.confirm('您确定要提交设置密码[' + userName + ']的角色信息', {
                ...
                $.ajax({
                    type: "POST",
                    data: {"userId": userId, "password": firstPwd},
...
function openUserRolePanel(userId){
    layer.open({
        ...
        shadeClose: true,
        area: ['550px',"600px"],
        content: '/sys/user/userRoleSet?userId='+userId //这里content是一个URL，如果你不想让iframe出现滚动条，你还可以content: ['http://sentsin.com', 'no']
        ,btn: ['保存', '关闭']
        ,btn1: function(index, layero){
        //按钮【按钮二】的回调
...
function getUserAndRoles(obj,id) {
	...
	if(obj.job){
	    ...
	    //回显数据
	    $.get("/sys/user/getUser",{"id":id},function(data){
	    	if(isLogin(data)){
