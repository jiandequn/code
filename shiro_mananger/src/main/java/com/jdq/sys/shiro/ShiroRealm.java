...
public class ShiroRealm extends AuthorizingRealm {
	...
	protected AuthorizationInfo doGetAuthorizationInfo(
		...
		if (user.getTelephone().equals("13692218270") || user.getUsername().equals("admin")|| user.getId() == 1) {
			// 超级管理员，添加所有角色、添加所有权限
			...
			if (null != roles && roles.size() > 0) {
				for (SysRole role : roles) {
					...
					// 角色对应的权限数据
					List<SysPermission> perms = this.authService.findPermsByRoleId(role
							.getId());
...
//		User user = userMapper.findUserByMobile(mobile);
		...
		LoginUser user = authService.getLoginUser(username);
		...
		if (user == null) {
			...
			// 第三个参数 ，realm名字
			return new SimpleAuthenticationInfo(user, DigestUtils.md5DigestAsHex(user.getPassword().getBytes()),
					getName());
		}
	}
