 ...
 * 按时间段统计历史播放记录日志文件
 */
public class AlbumPlayLogApp extends Configured implements Tool {
    static final String OUT_PATH = "album_play_log";
    ...
    public static void main(String[] args) throws Exception {
        ...
        int exitCode = ToolRunner.run(new AlbumPlayLogApp(), args1);
    ...
    public void exce(ConfigEntity configEntity) {
        try{
            ...
            if(!"success".equals(HttpClientUtils.doGet(url))){
                System.out.println(AlbumPlayLogApp.class+"接口初始化失败，表："+tableName);
                return;
            }
    ...
    public ControlledJob addMysqlJob(String jobName,ConfigEntity configEntity) throws Exception {
        ...
        job.setMapperClass(AlbumPlayLogMapper.class);
        job.setMapOutputKeyClass(AlbumPlayLogWritable.class);
        job.setMapOutputValueClass(NullWritable.class);
        job.setOutputKeyClass(AlbumPlayLogWritable.class);
        ...
        String[] fieldName = {"parent_column_id","user_type","mac","sn","user_id","column_id","album_id","video_id","content_type","time_position","create_time"};
