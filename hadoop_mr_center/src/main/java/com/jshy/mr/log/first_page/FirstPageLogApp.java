 ...
 * 按时间段统计历史播放记录日志文件
 */
public class FirstPageLogApp extends Configured implements Tool {
    static final String OUT_PATH = "first_page_log";
    ...
    public static void main(String[] args) throws Exception {
        ...
        int exitCode = ToolRunner.run(new FirstPageLogApp(), args1);
    ...
    public void exce(ConfigEntity configEntity) {
        try{
            ...
            if(!"success".equals(HttpClientUtils.doGet(url))){
                System.out.println(FirstPageLogApp.class+"接口初始化失败，表："+tableName);
                return;
            }
    ...
    public ControlledJob addMysqlJob(String jobName,ConfigEntity configEntity) throws Exception {
        ...
        job.setMapperClass(FirstPageLogMapper.class);
        job.setMapOutputKeyClass(FirstPageLogWritable.class);
        job.setMapOutputValueClass(NullWritable.class);
        job.setOutputKeyClass(FirstPageLogWritable.class);
        ...
        String[] fieldName = {"parent_column_id","user_type","mac","sn","user_id","column_id","after_column_id","after_column_code","area_code","create_time"};
