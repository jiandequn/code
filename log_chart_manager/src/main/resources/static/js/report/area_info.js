    ...
    layui.use('table', function(){
        ...
        tableIns=table.render({
            ...
            ,cols: [[
                ...
                ,{field:'cityNo', title:'城市编码'}
                ,{field:'cityName', title:'城市名称'}
                ,{field:'areaNo', title:'区域编码'}
        ...
        table.on('tool(areaInfoTable)', function(obj){
            var data = obj.data;
            if(obj.event === 'del'){
                del(data,data.id,data.areaName);
            } else if(obj.event === 'edit'){
                //编辑
                getAreaInfoInfo(data,data.id);
            }

        });
        //监听提交
        form.on('submit(areaInfoSubmit)', function(data){
            // TODO 校验
            $.ajax({
                type: "POST",
                data: $("#areaInfoForm").serialize(),
                url: "save",
...
function getAreaInfoInfo(obj,id) {
