...
var getMenus = function (data) {
    //回显选中
    //var ul = $("<ul id='sysNavTree' class='layui-nav layui-nav-tree' lay-filter='leftPermTreeFilter' style=''></ul>");
    var ul=$("#sysNavTree");
    for (var i = 0; i < data.length; i++) {
        var node = data[i];
        if (node.type == 0) {
            if (node.parentId == 0) {
                var li = $("<li class='layui-nav-item' flag='" + node.id + "' style='width: 300px'></li>");
                //父级无page
