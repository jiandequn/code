...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports){
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      if(!sort && that.sortCols){
        return that.sort(that.sortCols, true);
      }
  ...
  Class.prototype.sort = function(sorts, pull, formEvent){
    ...
    if (sorts.length == that.sortCols.length && !pull) {
      var sorted = true;
  ...
  Class.prototype.events = function(){
            ...
            openPanel({
              ...
              ,done: function(panel, list){
                list.on('click', function(){
                  ...
                  layui.layer.confirm('下载方式', {
                    btn: ['远程服务下载','本地下载'] //按钮
                  }, function(){
                    var index = layer.load(2, {time: 20*1000}); //又换了种风格，并且设定最长等待10秒
                    $.ajax({
                      type: options.method || 'get'
                      ,url: options.url
                      ,contentType: options.contentType
                      ...
                      ,success: function(res){
                        ...
                        if(res[options.response.statusName] != options.response.statusCode){
                          layer.msg('返回的数据不符合规范，正确的成功状态码应为："'+ options.response.statusName +'": '+ options.response.statusCode);
                        } else {
                          // that.renderData(res, curr, res[response.countName]), sort();
                          var data = res[options.response.dataName] || [] //列表数据
                          table.exportFile.call(that, options.id, data, type);
    ...
    th.on('click', function(e){
      ...
      // 单列排序配置
      if (!that.config.mutlSort && type != null)
        sorts = [{ field: field, type: type }];
