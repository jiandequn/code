...
/**
 * 用户来源按日统计(AppInletPositionCountDay)表控制层
 *
 * @author jdq
 * @since 2021-05-25 09:56:29
 */
@RestController
@RequestMapping("/report/appInletPositionCountDay")
public class AppInletPositionCountDayController extends ApiController {
    private static final Logger logger = LoggerFactory
            .getLogger(AppInletPositionCountDayController.class);
    @Autowired
    private AppInletPositionCountDayService appInletPositionCountDayService;
    /**
     * 日访问数据页面
     * @return ok/fail
     */
    ...
    public ModelAndView toPage() {
        logger.debug("权限列表！");
        ModelAndView mav = new ModelAndView("/report/app/inlet_position_count_day");
        return mav;
    }
    @RequestMapping("list")
    public ResultPage getList(String userType,String parentColumnId, String spm, String startDate, String endDate, Integer page, Integer limit, String orderby) {
        QueryWrapper<AppInletPositionCountDay> qw = new QueryWrapper();
        ...
        if(page ==null || limit == null){
            List list = this.appInletPositionCountDayService.list(qw);
            return new ResultPage(list.size(), list);
        }
        Page<AppInletPositionCountDay> pg =new Page(page,limit,true);
        return  ResultPage.fromMybatisPlusPage(appInletPositionCountDayService.page(pg,qw));
    }
    ...
    public Map<String,Object> getChartList(String userType,String parentColumnId, String startDate, String endDate) {
        ...
        Optional.ofNullable(endDate).filter(s -> !s.isEmpty()).ifPresent(t->qw.le("t_date",endDate));
        Optional.ofNullable(userType).filter(s -> !s.isEmpty()).ifPresent(t->qw.eq("user_type",userType));
        List<AppInletPositionCountDay> userList =this.appInletPositionCountDayService.list(qw);
...
//        return result;
    ...
    public void chartExport(String fileName, HttpServletResponse response,String userType, String parentColumnId, String startDate, String endDate)
        ...
        Optional.ofNullable(userType).filter(s -> !s.isEmpty()).ifPresent(t->qw.eq("user_type",userType));
        List<AppInletPositionCountDay> userList =this.appInletPositionCountDayService.list(qw);
        EasyExcel.write(out, AppInletPositionCountDay.class).sheet("专区内专辑类型日统计").doWrite(userList);
    }
}
