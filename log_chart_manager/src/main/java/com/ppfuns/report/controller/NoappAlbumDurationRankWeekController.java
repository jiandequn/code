...
/**
 * 所有专区专辑播放时长周排行榜(NoappAlbumDurationRankWeek)表控制层
 *
 * @author jdq
 * @since 2021-07-14 15:21:58
 */
@RestController
@RequestMapping("/report/noapp_album_duration_rank_week")
public class NoappAlbumDurationRankWeekController extends ChartController {
    /**
     * 服务对象
     */
    @Resource
    private NoappAlbumDurationRankWeekService noappAlbumDurationRankWeekService;
    @RequestMapping(value = "/toPage", method = RequestMethod.GET)
    public ModelAndView toPage() {
        logger.debug("权限列表！");
        ModelAndView mav = new ModelAndView("/report/noapp/album_duration_rank/week");
        return mav;
    }
    @RequestMapping("/list")
    public ResultPage getList(String userType, String week, Integer page, Integer limit, String orderby) {
        QueryWrapper<NoappAlbumDurationRankWeek> qw = new QueryWrapper();
        ...
        if(page ==null || limit == null){
            List list = this.noappAlbumDurationRankWeekService.list(qw);
            return new ResultPage(list.size(), list);
        }
        Page<NoappAlbumDurationRankWeek> pg =new Page(page,limit,true);
        return  ResultPage.fromMybatisPlusPage(noappAlbumDurationRankWeekService.page(pg,qw));
    }
    ...
    public Map<String,Object> getChartList(String userType,Integer rankLimit, String countFields,String countFieldTypes, String week) {
        ...
        return super.rankData(noappAlbumDurationRankWeekService,qw,rankLimit,countFields,countFieldTypes);
    }
}
