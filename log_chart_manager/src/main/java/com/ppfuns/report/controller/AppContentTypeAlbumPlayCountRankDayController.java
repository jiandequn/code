...
/**
 * 专区类型下专辑播放次数日排行榜(AppContentTypeAlbumPlayCountRankDay)表控制层
 *
 * @author jdq
 * @since 2021-07-06 16:58:13
 */
@RestController
@RequestMapping("/report/app_content_type_album_play_count_rank_day")
public class AppContentTypeAlbumPlayCountRankDayController extends ChartController {
    /**
     * 服务对象
     */
    @Resource
    private AppContentTypeAlbumPlayCountRankDayService appContentTypeAlbumPlayCountRankDayService;

    @RequestMapping(value = "/toPage", method = RequestMethod.GET)
    public ModelAndView toPage() {
        logger.debug("权限列表！");
        ModelAndView mav = new ModelAndView("/report/app/content_type_album_play_count_rank/day");
        return mav;
    }
    @RequestMapping("/list")
    public ResultPage getList(String parentColumnId,String contentType, String date, Integer page, Integer limit, String orderby) {
        QueryWrapper<AppContentTypeAlbumPlayCountRankDay> qw = new QueryWrapper();
        ...
        if(page ==null || limit == null){
            List list = this.appContentTypeAlbumPlayCountRankDayService.list(qw);
            return new ResultPage(list.size(), list);
        }
        Page<AppContentTypeAlbumPlayCountRankDay> pg =new Page(page,limit,true);
        return  ResultPage.fromMybatisPlusPage(appContentTypeAlbumPlayCountRankDayService.page(pg,qw));
    }
    ...
    public Map<String,Object> getChartList(String parentColumnId,Integer rankLimit, String countFields,String countFieldTypes, String date,String contentType) {
        ...
        return super.rankData(appContentTypeAlbumPlayCountRankDayService,qw,rankLimit,countFields,countFieldTypes);
    }
}
