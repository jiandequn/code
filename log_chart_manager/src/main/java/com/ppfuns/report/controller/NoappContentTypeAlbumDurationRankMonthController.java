...
/**
 * 所有专区类型下专辑播放时长月排行榜(NoappContentTypeAlbumDurationRankMonth)表控制层
 *
 * @author jdq
 * @since 2021-07-14 15:21:59
 */
@RestController
@RequestMapping("/report/noapp_content_type_album_duration_rank_month")
public class NoappContentTypeAlbumDurationRankMonthController extends ChartController {
    /**
     * 服务对象
     */
    @Resource
    private NoappContentTypeAlbumDurationRankMonthService noappContentTypeAlbumDurationRankMonthService;
    @RequestMapping(value = "/toPage", method = RequestMethod.GET)
    public ModelAndView toPage() {
        logger.debug("权限列表！");
        ModelAndView mav = new ModelAndView("/report/noapp/content_type_album_duration_rank/month");
        return mav;
    }
    @RequestMapping("/list")
    public ResultPage getList(String userType,String contentType, String startDate, Integer page, Integer limit, String orderby) {
        QueryWrapper<NoappContentTypeAlbumDurationRankMonth> qw = new QueryWrapper();
        ...
        if(page ==null || limit == null){
            List list = noappContentTypeAlbumDurationRankMonthService.list(qw);
            return new ResultPage(list.size(), list);
        }
        Page<NoappContentTypeAlbumDurationRankMonth> pg =new Page(page,limit,true);
        return  ResultPage.fromMybatisPlusPage(noappContentTypeAlbumDurationRankMonthService.page(pg,qw));
    }
    ...
    public Map<String,Object> getChartList(String userType, String contentType, Integer rankLimit, String countFields,String countFieldTypes,String startDate) {
        ...
        qw.orderByAsc("user_type,content_type,rank");
        return super.rankData(noappContentTypeAlbumDurationRankMonthService,qw,rankLimit,countFields,countFieldTypes);
    }
}
