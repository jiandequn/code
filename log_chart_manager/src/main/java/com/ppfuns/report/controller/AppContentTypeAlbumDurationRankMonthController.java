...
/**
 * 专区类型下专辑播放时长月排行榜(AppContentTypeAlbumDurationRankMonth)表控制层
 *
 * @author jdq
 * @since 2021-07-14 15:20:46
 */
@RestController
@RequestMapping("/report/app_content_type_album_duration_rank_month")
public class AppContentTypeAlbumDurationRankMonthController extends ChartController {
    /**
     * 服务对象
     */
    @Resource
    private AppContentTypeAlbumDurationRankMonthService appContentTypeAlbumDurationRankMonthService;

    @RequestMapping(value = "/toPage", method = RequestMethod.GET)
    public ModelAndView toPage() {
        logger.debug("权限列表！");
        ModelAndView mav = new ModelAndView("/report/app/content_type_album_duration_rank/month");
        return mav;
    }
    @RequestMapping("/list")
    public ResultPage getList(String parentColumnId,String contentType, String startDate, Integer page, Integer limit, String orderby) {
        QueryWrapper<AppContentTypeAlbumDurationRankMonth> qw = new QueryWrapper();
        ...
        if(page ==null || limit == null){
            List list = appContentTypeAlbumDurationRankMonthService.list(qw);
            return new ResultPage(list.size(), list);
        }
        Page<AppContentTypeAlbumDurationRankMonth> pg =new Page(page,limit,true);
        return  ResultPage.fromMybatisPlusPage(appContentTypeAlbumDurationRankMonthService.page(pg,qw));
    }
    ...
    public Map<String,Object> getChartList(String parentColumnId, String contentType,Integer rankLimit, String countFields,String countFieldTypes, String startDate) {
        ...
        qw.orderByAsc("parent_column_id,content_type,rank");
        return super.rankData(appContentTypeAlbumDurationRankMonthService,qw,rankLimit,countFields,countFieldTypes);
    }
}
